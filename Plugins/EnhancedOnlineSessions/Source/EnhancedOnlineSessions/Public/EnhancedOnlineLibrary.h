...
class UEnhancedOnlineRequest_SearchSessions;
...
DECLARE_DYNAMIC_DELEGATE_OneParam(FBlueprintOnFindSessionsSuccess, const TArray<UEnhancedSessionSearchResult*>&, SessionResults);
...
public:
	...
	/**
	 * Creates a search session request object that can be used to search for sessions.
	 * 
	 * @param WorldContextObject			The world context object, IF YOU SEE THIS IN BLUEPRINTS, YOU ARE DOING SOMETHING WRONG >:(
	 * @param OnlineMode					The online mode of the session
	 * @param MaxSearchResults				The maximum number of search results that will be returned, default to -1 for no limit
	 * @param bSearchLobbiesIfAvailable		Should lobbies be searched if the online service supports them
	 * @param SearchKeyword					The keyword that will be used to search for the session, default to empty string means all sessions
	 * @param LocalUserIndex				The index of the local user that is making the request
	 * @param bInvalidateAfterComplete		Should the request be invalidated after it is completed, default to true to save memory
	 * @param OnSuccess						The delegate that will be called if the request succeeds
	 * @param OnFailed						The delegate that will be called if the request fails
	 * @return The search session request object
	 */
	UFUNCTION(BlueprintCallable, BlueprintPure, Category = "Online|EnhancedSessions|Sessions", meta =
		(WorldContext = "WorldContextObject", Keywords = "Create, Make, New", AdvancedDisplay = "bInvalidateAfterComplete",
			OnlineMode = "Online", MaxSearchResults = "-1", bSearchLobbiesIfAvailable = "true", bInvalidateAfterComplete = "true", LocalUserIndex = "0"))
	static UPARAM(DisplayName = "Request") UEnhancedOnlineRequest_SearchSessions* ConstructOnlineSearchSessionsRequest(
		UObject* WorldContextObject,
		const EEnhancedSessionOnlineMode OnlineMode,
		int32 MaxSearchResults,
		bool bSearchLobbiesIfAvailable,
		FString SearchKeyword,
		int32 LocalUserIndex,
		bool bInvalidateAfterComplete,
		FBlueprintOnFindSessionsSuccess OnSuccess,
		FBlueprintOnRequestFailedWithLogin OnFailed);
